					...
					<?php
						foreach ($page_nav as $key => $nav_item) {
							...
							$label_htm = isset($nav_item["label_htm"]) ? $nav_item["label_htm"] : "";
                                                        if($url=="#"){
                                                            $nav_htm .= '<a href="javascript:void(0)" '.$url_target.' title="'.$nav_title.'">'.$icon.' <span class="menu-item-parent">'.$nav_title.'</span>'.$label_htm.'</a>';
                                                        }else{
                                                            $nav_htm .= '<a href="'.ASSETS_URL."/backend/".$url.'" '.$url_target.' title="'.$nav_title.'">'.$icon.' <span class="menu-item-parent">'.$nav_title.'</span>'.$label_htm.'</a>';
                                                        }
								...
								foreach ($nav_item as $key => $sub_item) {
									...
									if($url=="#"){
										...
										'<li '.(isset($sub_item["active"]) ? 'class = "active"' : '').'>
											<a href="javascript:void(0)" '.$url_target.'>'.$icon.' '.$nav_title.$label_htm.'</a>
											'.(isset($sub_item["sub"]) ? process_sub_nav($sub_item["sub"]) : '').'
                                                                            ...
                                                                            $sub_item_htm .= 
										'<li '.(isset($sub_item["active"]) ? 'class = "active"' : '').'>
											<a href="'.ASSETS_URL."/backend/".$url.'" '.$url_target.'>'.$icon.' '.$nav_title.$label_htm.'</a>
											'.(isset($sub_item["sub"]) ? process_sub_nav($sub_item["sub"]) : '').'
										</li>';
                                                                        }
