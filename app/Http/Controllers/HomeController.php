...
class HomeController extends Controller
{
    ...
    public function getAboutUs(){
        $post = Post::where("permalink","LIKE","about%")->first();
        return View("aboutus",["mypage"=>$post]);
    }

    public function getOrderForm($bagtitle=""){
        return View("orderform",["bag_title"=>$bagtitle]);
    }

    public function getPages($permalink){
        $post = Post::where("permalink","=",$permalink)->first();
        return View("pages.pages",["mypage"=>$post]);
    }

    public function postOrderForm( Request $request,$bagtitle=""){
        if($request->ajax()){

            if(isset($_POST['faction']) && ($_POST['faction']=="logo")){

                ############ Configuration ##############
                $thumb_square_size 		= 200; //Thumbnails will be cropped to 200x200 pixels
                $max_image_size 		= 500; //Maximum image size (height and width)
                $destination_folder = public_path()."/uploads/docs/";
                ...
                if(isset($_POST) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
                    ...
                    if($image_res){
                        ...
                        if(normal_resize_image($image_res, $image_save_folder, $image_type, $max_image_size, $image_width, $image_height, $jpeg_quality))
                        {
                            ...
                            echo '<img src="'.public_path().'/uploads/docs/'.$new_file_name.'" alt="Resized Image">';
